-- CUSTOM UI: Millionaire Empire Tycoon (Scriptyyz Edition)
...
-- Globals
getgenv().InfMoneyLoop = false
local rs = game:GetService("ReplicatedStorage")
local remote = rs:WaitForChild("fewjnfejwb3")
...
MainFrame.Size = UDim2.new(0, 250, 0, 260) -- Smanjena visina da nema praznog prostora
...
MinimizeBtn.MouseButton1Click:Connect(function()
    ...
    MainFrame:TweenSize(minimized and UDim2.new(0, 250, 0, 45) or UDim2.new(0, 250, 0, 260), "Out", "Quart", 0.3, true)
...
Footer.Text = "Millionaire Empire Tycoon" -- Promenjeno ovde
...
createToggle("Infinite Money", function(state)
    getgenv().InfMoneyLoop = state
    if state then
        task.spawn(function()
            while getgenv().InfMoneyLoop do
                pcall(function()
                    remote:FireServer(unpack({160000000000000}))
                end)
                task.wait(0.15)
            end
        end)
...
createButton("Quick 50x Money Burst", function()
    for i = 1, 50 do
        pcall(function() remote:FireServer(unpack({160000000000000})) end)
        task.wait(0.02)
    end
end)
