...
local BASE_POS = CFrame.new(41.60, -7.82, -23.04)
...
Footer.Text = "Rope Bridge for Brainrots" -- Izmenjeno po tvom zahtevu
    ...
    autoFarmLoop = task.spawn(function()
        while autoFarming do
            ...
            if root then
                local target = nil
                for _, v in pairs(Workspace:GetDescendants()) do
                    if v:IsA("BasePart") and v.Name == "ServerHitbox" then
                        target = v
                        break
                    end
                end
